...
sync
start_writing=$(date +%s%N)

# --- Código a medir ---
echo "1In the beginning God created the heavens and the earth. 2Now the earth was formless and empty, darkness was over the surface of the deep, and the Spirit of God was hovering over the waters.
...
# --- Código a medir ---
cat DiskOperationSample.txt > /dev/null
rm DiskOperationSample.txt
# ----------------------

end_reading=$(date +%s%N)
sync

elapsed_reading=$(((end_reading - start_reading)/1000000))

read l w m c < <(wc -l -w -m -c < SampleMemoryWriting.txt)

echo "Se usaron $w palabras, $l líneas, $m caracteres y $c bytes."
echo "Tiempo transcurrido: $elapsed_writing ms"
echo "Tiempo transcurrido: $elapsed_reading ms"
